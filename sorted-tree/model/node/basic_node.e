...
feature -- queries
	...
	is_equal (other: like Current): BOOLEAN
		-- return the keys are same or not
		do
 			Result := key ~ other.key
		end
	...
	sibling: detachable like Current
		...
		if attached parent as p then
			...
			correct_sibling:
				...
				((p.right = Result and p.left = Current)
				xor (p.left = Result and p.right = Current))
		end
...
feature -- inorder traversal from Current
	...
	inorder: STRING
		...
		do
			Result := out
			-- base on the invariant, Current is the parent right now
			-- split the Current for left and right children
			-- and the order is inorder so left child always presents first inside a parent node
			if attached Current.left as l_left then
				Result := l_left.inorder + Result
			end
			if attached Current.right as l_right then
				Result := Result + l_right.inorder
			end
